...
function App() {
  ...
  const [filteredData, setFilteredData] = useState([]);
    ...
    //Define the Table Header properties used in Table Component
    const columns = [
        {
            Header: "",
            accessor: "id",
    ...
    // Update the select state when itmes selected
    const onSelectChange = (e) => {
        if (e.currentTarget.checked) {
            //alert(e.target.value);
             if (filteredData.length > 0) {   
                selected.push(filteredData[e.target.value].code);
             } else {
                selected.push(rowdata[e.target.value].code); 
             }
        }else{
            if (filteredData.length > 0) {   
                const index = selected.indexOf(filteredData[e.target.value].code);
                selected.splice(index,1);
            } else {
                const index = selected.indexOf(rowdata[e.target.value].code);
                selected.splice(index,1);
            }
        }
    };
    ...
    //Return the codes of countries in selected state
    ...
    return (
        <div className="App">
            ...
            <div className="flex justify-center mt-8">
                <Button value="View Selected Codes" onClick={handleSubmit} />
            </div>
            <br /> <br />  
